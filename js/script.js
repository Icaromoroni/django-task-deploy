const baseURL = 'https://django-server-production-6f83.up.railway.app/tarefas/'
...
function atualizar_tela(){
  // criação da tabela para informar as tarefas como se fosse uma lista
  const tabela = document.getElementById('table-task');
  const form_task = document.getElementById('task-form');
  tabela.innerHTML = []
  
  for(let tarefa of tarefas ) {
    //criar tr, th, 3*td, 3*a, 3*img
    ...
    //configurando botão link editar
    a_editar.href='#'
    ...
    img_editar.src = 'img/editar.png'
    img_atualizar.src = 'img/atualizar.png'
    img_remover.src = 'img/lixo.png'
    ...
    for (let i = 0; i < linksVisualizar.length; i++) {
      linksVisualizar[i].addEventListener('click', async (event) => {
        event.preventDefault();
        // visualizar a tarefa correspondente
